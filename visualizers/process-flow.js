...
const ExtractData = (filepath) => {
    ...
    for(currCol = 0; currCol <= range.e.c; currCol++) {
        ...
        const dataArray = [];
        for(currRow = 1; currRow < range.e.r; currRow++) {
...
const CreateProcessFlow = (processedData) => {
    ...
    let nodes = ``;
    for(i = 0; i < nodeMessages.length; i++) {
        nodes += `node${i} [label="${nodeMessages[i]}"]\n\t\t`;
    }
    for(i = 0; i < nodeMessages.length-1; i++) {
        nodes += `\n\t\tnode${i} -> node${i+1}`;
    }
...
CreateProcessFlow(ExtractData("visualizers/mock_data.xlsx"));